...
def create_tryptic_peptides(fasta_file, min_length=7, max_length=30):
    peptides = []
    ...
    peptides = list(set(peptides))  # Remove duplicates
    peptides = [p for p in peptides if len(p) >= min_length and len(p) <= max_length]
    return peptides
