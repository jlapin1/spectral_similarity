...
def modified_dot_product(mz1, intensity1, mz2, intensity2, mz_weight=1, **kwargs):
    ...
    return np.sum(weights * intensity1 * intensity2) / (
        np.sqrt(np.sum(weights * intensity1**2))
        * np.sqrt(np.sum(weights * intensity2**2))
    )


# MASSBANK score (simplified)
def massbank_score(intensity1, intensity2, **kwargs):
    score = np.sum(intensity1 * intensity2) / (
        np.sum(intensity1**2) + np.sum(intensity2**2) - np.sum(intensity1 * intensity2)
    )
    return score
...
def stein_scott_score(intensity1, intensity2, **kwargs):
    ...
    score = np.sum(prod) / (
        np.sqrt(np.sum(intensity1**2)) * np.sqrt(np.sum(intensity2**2))
    )
    return score
...
def diagnostic_weighted_similarity(
    mz, intensity1, intensity2, diagnostic_mz, diagnostic_weight=2, **kwargs
):
